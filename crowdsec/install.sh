...
# ---------------------------------------------------------------------------
# 2. Download required repository files
# ---------------------------------------------------------------------------
...
download_file "config/acquis.d/nginx-proxy.yaml"
download_file "config/acquis.d/sshd.yaml"
download_file "config/parsers/s02-enrich/whitelists.yaml"

log "All required repository files downloaded."

# ---------------------------------------------------------------------------
# 3. Detect EasyEngine nginx-proxy log directory
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Copy downloaded repository files
# ---------------------------------------------------------------------------

copy_repo_file() {
    local src="$DOWNLOAD_DIR/$1"
    local dst="$APP_DIR/$1"
    local mode="$2"

    [[ -f "$src" ]] \
        || die "Missing downloaded repository file: $1"
